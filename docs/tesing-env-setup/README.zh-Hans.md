...
1. 程序的第 1 行用于指定接下来的汇编的结果（即二进制指令）放在目标文件的哪个段；
2. 接下来的两句 `.globl` 用于声明两个导出符号（在这里也就是函数的名称，最后导出的是函数的地址）；
3. `_start` 可以看作是一个函数，它会调用另一个函数 `_print_a`，然后进入无限循环（汇编语言实际上没有函数概念，也就是说结构之间没有边界，为了防止 `_start` 函数继续往下执行，所以在函数的末尾处设了一个无限循环）；
